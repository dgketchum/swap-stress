"""van Genuchten curve fitting from paired (suction, theta) observations.

Restored from ``retention_curve/swrc.py``, which was deleted in 39946a5. The
fits it produces are what the retention-curve figures and the Technical
Validation comparison are drawn from.

Two fitting paths:

- :meth:`SWRC.fit` -- deterministic least squares via lmfit. Pass
  ``method='nelder'``; this is what the archived ``nelder/`` fit JSONs were
  produced with.
- :meth:`SWRC.fit_bayesian` -- NUTS via pymc, giving posterior intervals on
  each parameter. Produced the archived ``bayes/`` JSONs.

The forward van Genuchten evaluation delegates to :mod:`swapstress.swrc` so
there is one implementation of the algebra. The pymc path necessarily rebuilds
it in pytensor ops, since the model must be differentiable and symbolic -- that
is the one place the equation is deliberately written twice, and
``tests/test_fitting.py`` pins the two against each other.
"""
...
class VGFitResultLight:
    ...
    def __init__(self, theta_r, theta_s, alpha, n):
        ...
        def Param(v):
            return type("Param", (), {"value": float(v), "stderr": None})()
    ...
    def eval(self, psi=None, **kwargs):
        ...
        # Saved fits can carry n marginally at the 1.001 bound; nudge above 1 so
        # a stored boundary fit still plots instead of coming back all-NaN.
        n = max(self._n, 1.0000001)
        return theta_from_psi(psi_arr, self._theta_r, self._theta_s, self._alpha, n)
...
class SWRC:
    ...
    def load_from_file(self, filepath):
        ...
        if "KPA" in df.columns and "VWC" in df.columns:
            ...
            raise ValueError(
                "Data must contain either ('suction_cm', 'theta') or ('KPA', 'VWC') columns."
            )
    ...
    def load_from_dataframe(self, df):
        ...
        if not has_suction or "theta" not in cols or not has_depth:
            raise ValueError(
                "DataFrame missing required columns: need ('suction'|'suction_cm'), 'theta', ('depth'|'depth_cm')"
            )
        ...
        meta_cols = [
            c for c in df.columns if c not in {"suction_cm", "theta", group_col}
        ]
    ...
    def _van_genuchten_model(psi, theta_r, theta_s, alpha, n):
        """Forward vG, the residual model lmfit optimizes against.

        ``strict=False`` is required, not incidental. The bounds set in
        :meth:`_generate_initial_params` are ``theta_r <= 1.1 * min(theta)`` and
        ``theta_s >= 0.9 * max(theta)``, which overlap whenever the observed
        theta range is narrow -- so Nelder-Mead does explore ``theta_r >
        theta_s``. Returning NaN there aborts the fit outright instead of
        steering the simplex back, and it is what the archived fits were
        produced against.
        """
        return theta_from_psi(psi, theta_r, theta_s, alpha, n, strict=False)
    ...
    def _generate_initial_params(self, data_for_depth):
        ...
        n_init = 1.5
        params["theta_s"].set(
            value=theta_s_init, min=theta_s_init * 0.9, max=theta_s_init * 1.1
        )
    ...
    def fit(self, report=False, method="nelder"):
        """
        ...
        Args:
            ...
            method (str): lmfit minimizer name. Defaults to 'nelder', which is
                what the archived fit JSONs were produced with. Passing
                'bayes'/'bayesian' routes to :meth:`fit_bayesian` instead.
    ...
    def fit_bayesian(
        ...
        for depth, data_df in self.data_by_depth.items():
            ...
            with pm.Model():
                ...
                cap = (
                    None
                    if theta_r_cap is None
                    else max(1e-6, min(float(theta_r_cap), 0.99))
                )
                upper_tr = cap if cap is not None else 0.05
                theta_r = pm.TruncatedNormal(
                    "theta_r", mu=0.01, sigma=0.05, lower=0.0, upper=upper_tr
                )
                ...
                theta_s = pm.TruncatedNormal(
                    "theta_s", mu=0.45, sigma=0.15, lower=lower_ts, upper=upper_val
                )
    ...
    def test_fit_methods(self, methods_to_test, depth=None):
        ...
        data_df = data_df[
            np.isfinite(data_df["suction"]) & np.isfinite(data_df["theta"])
        ]
    ...
    def plot(self, save_path=None, show=True, colormap="plasma"):
        ...
        colors = plt.cm.get_cmap(colormap)(
            np.linspace(0, 0.85, len(self.data_by_depth))
        )

        for i, (depth, data_df) in enumerate(self.data_by_depth.items()):
            color = colors[i]
            label_prefix = (
                f"Depth {depth} cm" if self.depth_col in data_df.columns else "Data"
            )
            ...
            if fit_result and fit_result.success:
                psi_smooth = np.logspace(
                    np.log10(max(1e-2, data_df["suction"].min())),
                    np.log10(data_df["suction"].max()),
                    200,
                )
    ...
    def results_summary(self):
        ...
        for depth, result in self.fit_results.items():
            ...
            df_raw = df_raw[
                np.isfinite(df_raw["suction"]) & np.isfinite(df_raw["theta"])
            ]
            data_blob = {
    ...
    def load_from_results_json(self, results_path):
        ...
        if (
            ...
            and isinstance(summary["metadata"], dict)
        ):
