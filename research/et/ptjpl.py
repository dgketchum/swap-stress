...
def _export_chunked_tables(
    ...
    data_chunk = img_chunk.reduceRegions(
...
def export_ptjpl_zonal_stats(
    ...
    if buffer:
        assert (
            df.crs != "EPSG:4326"
        )  # running the buffer on geographic will blow up the area
        df.geometry = df.geometry.buffer(buffer)
    ...
    remap = (
        irr_coll.filterDate(s, e).select("classification").map(lambda img: img.lt(1))
    )
    ...
    for fid, row in tqdm(
        df.iterrows(), desc="Export PTJPL zonal stats", total=df.shape[0]
    ):
        if row["geometry"].geom_type == "Point":
            raise ValueError
            ...
            if polygon_asset:
                ...
                fc = ee.FeatureCollection(polygon_asset).filterMetadata(
                    feature_id, "equals", fid
                )
                polygon = fc.geometry()
            else:
                polygon = ee.Geometry(row.geometry.__geo_interface__)
