...
def _load_empirical_results(results_dir):
    ...
    json_files = glob(
        os.path.join(results_dir, "**", "*_fit_results.json"), recursive=True
    )
    if not json_files:
    ...
    df_m = df.melt(
        ...
        value_vars=[f"station_{p}" for p in params],
        var_name="param",
        value_name="value",
    )
    df_m["level_param"] = df_m.apply(
        lambda r: f"L{int(r['rosetta_level'])}_{r['param']}", axis=1
    )
    wide = df_m.pivot_table(
        index="station", columns="level_param", values="value", aggfunc="mean"
    ).reset_index()
    # Rename "Lx_station_param" to "station_Lx_param"
    wide.columns = [
        re.sub(r"^L([1-7])_station_", r"station_L\1_", c) for c in wide.columns
    ]
    return wide
...
def _load_rosetta_params(rosetta_parquet):
    ...
    df.columns = [
        re.sub(r"US_R3H3_L([1-7])_VG_", r"rosetta_L\1_", c) for c in df.columns
    ]
    return df
...
def _load_gshp_params(csv_path):
    ...
    g = (
        df[present]
        .groupby("layer_id")
        .agg({c: "first" for c in present if c != "layer_id"})
        .reset_index()
    )
    return {
        "theta_r": g["thetar"].dropna().values
        if "thetar" in g.columns
        else np.array([]),
        "theta_s": g["thetas"].dropna().values
        if "thetas" in g.columns
        else np.array([]),
...
def compare_parameter_distributions_combined(
    empirical_results_dir,
    rosetta_parquet,
    training_parquet,
    output_dir,
    gshp_csv=None,
    rosetta_training_parquet=None,
    rosetta_swrc_results_dir=None,
    polaris_all_depths_parquet=None,
    ...
    polaris_all_vals = (
        _load_polaris_all_depths(polaris_all_depths_parquet)
        if polaris_all_depths_parquet
        else {}
    )
    inferred_df = (
        pd.read_parquet(predictions_parquet) if predictions_parquet else pd.DataFrame()
    )
    gshp_vals = _load_gshp_params(gshp_csv) if gshp_csv else {}
    rosetta_training_vals = (
        _load_rosetta_training_params(rosetta_training_parquet)
        if rosetta_training_parquet
        else {}
    )
    rosetta_swrc_vals = (
        _load_rosetta_swrc_params(rosetta_swrc_results_dir)
        if rosetta_swrc_results_dir
        else {}
    )
    ...
    palette = {
        "Station (Obs)": "#ff7f0e",
        "Station (Inferred)": "#17becf",
        "Rosetta (Obs)": "#2ca02c",
        "GSHP (Obs)": "#9467bd",
        "Rosetta (Inferred)": "#d62728",
    ...
    for param in params:
        ...
        st_cols = [
            c for c in station_df.columns if re.match(rf"^station_L\d+_{param}$", c)
        ]
        st_series = (
            pd.concat([station_df[c] for c in st_cols], ignore_index=True)
            if st_cols
        ...
        ros_lin_cols = [
            c for c in ros_df.columns if re.match(rf"^rosetta_L2_{param}$", c)
        ]
        ros_log_cols = [
            c for c in ros_df.columns if re.match(rf"^rosetta_L2_log10_{param}$", c)
        ]
        ros_vals = []
        if ros_lin_cols:
        ...
        if ros_log_cols:
            ...
            if param in need_log:
                ...
                pass
        ros_series = (
            pd.concat(ros_vals, ignore_index=True)
            if ros_vals
            else pd.Series(dtype=float)
        )
        ...
        # GSHP (overall)
        g_series = (
            pd.Series(gshp_vals.get(param, [])) if gshp_vals else pd.Series(dtype=float)
        )
        ...
        rt_series = (
            pd.Series(rosetta_training_vals.get(param, []))
            if rosetta_training_vals
            else pd.Series(dtype=float)
        )
        ...
        # Rosetta (SWRC fits)
        rs_series = (
            pd.Series(rosetta_swrc_vals.get(param, []))
            if rosetta_swrc_vals
            else pd.Series(dtype=float)
        )
    ...
    if not all_sources_dfs:
        ...
        for i, p in enumerate(param_order):
            ...
            order = [s for s in desired if s in present] + [
                s for s in present if s not in desired
            ]
            pal_list = [
                palette.get(s, "#333333") for s in order
            ]  # likely error: palette missing renamed source label
            sns.boxplot(
                data=sdf, x="Source", y="Value", order=order, palette=pal_list, ax=ax
            )
...
if __name__ == "__main__":
    ...
    rosetta_training_pqt = os.path.join(
        rosetta_soil_proj, "rosetta", "db", "Data.parquet"
    )

    # Combined all-depths, single figure with 4 panels
    gshp_csv = os.path.join(
        root, "soil_potential_obs", "gshp", "WRC_dataset_surya_et_al_2021_final.csv"
    )
    compare_parameter_distributions_combined(
        empirical_dir,
        rosetta_pqt,
        training_pqt,
        out_dir,
        gshp_csv=gshp_csv,
        rosetta_training_parquet=rosetta_training_pqt,
