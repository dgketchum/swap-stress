...
def _plot_vwc_with_flags(
    ts, meta_dict, out_png, vwc_col="soil_moisture", flag_col="soil_moisture_flag"
):
    ...
    title = (
        f"{network}:{station} — {dmid:.2f} m"
        if dmid is not None
        else f"{network}:{station}"
    )
    ax.set_title(title)
...
def _daily_series(
    ts, prefer_flagged=True, vwc_col="soil_moisture", flag_col="soil_moisture_flag"
):
    df = ts.copy()
...
def _process_dataset_id(idx, out_dir, min_depth_m, max_depth_m, plot):
    ...
    if plot:
        ...
        plot_fp = os.path.join(
            plots_dir, safe_network, f"{safe_station}_{depth_cm}cm.png"
        )
    ...
    try:
        ...
        out_df = out_df[
            ["datetime", "station", "latitude", "longitude"]
            + sorted(vwc_cols, key=lambda x: int(x.split("_")[-1]))
        ]
...
def build_ismn_vwc_series(
    ...
    with ProcessPoolExecutor(
        ...
        futures = [
            ex.submit(_process_dataset_id, idx, out_dir, min_depth_m, max_depth_m, plot)
            for idx in ids
        ]
        for f in tqdm(
            as_completed(futures), total=len(futures), desc="Processing ISMN datasets"
        ):
            try:
    ...
    try:
        n_station_files = len(
            [fn for fn in os.listdir(station_dir) if fn.endswith(".parquet")]
        )
    except Exception:
        n_station_files = 0
