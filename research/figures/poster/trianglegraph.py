...
class SoilTrianglePlot:
    ...
    def _toCart(self, threecoords):
        ...
        global type
        global sqrt
        assert type(threecoords) == type([]), (
            "Expected a list of 3-coordinate tuple points"
        )
        cartxs = []
        cartys = []
        for triple in threecoords:
            (bottom, left, right) = triple
            assert bottom + left + right == 100, (
                "3-coordinate values must sum to 100; %d, %d, %d don't"
                % (
                    bottom,
                    left,
                    right,
            ...
            cartxs.append(100 - bottom - left / 2.0)
            cartys.append(sqrt(3) * left / 2.0)
    ...
    def grid(self, triple=([25, 50, 100], [25, 50, 100], [25, 50, 100]), labels=()):
        ...
        global sqrt
        (bs, ls, rs) = triple
        lstyle = {"color": "0.6", "dashes": (1, 1), "linewidth": 0.5}
        for b in bs:
        ...
        for left in ls:
            assert 0 <= left <= 100, "Left value not in 0-100 range"
            self.line((0, left, 100 - left), (100 - left, left, 0), **lstyle)
            p.text(
                left / 2.0 - 2 * len(str(left)),
                sqrt(3) * left / 2 - 1,
                left,
                fontsize=9,
            )
    ...
    def soil_categories(self, sclabel="_nolegend_", country="USA"):
        ...
        p.text(
            80 - len(labels[2]) / 2, 50 - len(labels[2]) / 2, labels[2], rotation=300
        )
    ...
    def patch(self, limits, **kwargs):
        ...
        for pt in bounds:  # plug in values for these limits
            ...
            for i in [0, 1, 2]:
                if pt[i] == -1:
                    pt[i] = 99 - sum(pt)
            if self.satisfies_bounds(pt, limits):
                coords.append(pt)
    ...
    def scatter_from_csv(
        ...
        **kwargs,
        ...
        if count < 3:
            print(
                "ERROR: need columns for sand, silt and clay identified in ', filename"
            )
