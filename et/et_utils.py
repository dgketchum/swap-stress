...
def add_doy(image):
    ...
    day = ee.Image.constant(image.date().getRelative("day", "year")).clip(
        image.geometry()
    )
...
def get_world_climate(proj, months, param="prec"):
    if months[0] > months[1]:
        months = [x for x in range(months[0], 13)] + [
            x for x in range(1, months[1] + 1)
        ]
    else:
    ...
    images = [
        ee.Image(f"WORLDCLIM/V1/MONTHLY/{m}")
        .select(param)
        .resample("bilinear")
        .reproject(crs=proj["crs"], scale=30)
        for m in months
    ]
    i = ee.ImageCollection(images)
    if param == "prec":
        i = i.sum()
    else:
        i = i.mean()
...
def long_term_ndvi(roi, coll=False):
    ...
    for yr in range(1987, 2023):
        ...
        ndvi_mx = ee.Image(
            landsat.filterDate(s, e)
            .map(lambda x: x.normalizedDifference(["B5", "B4"]))
            .max()
        ).rename("nd_max")
    ...
    mean_, max_, std_ = (
        coll.mean().rename("nd_lt_mn"),
        coll.max().rename("nd_lt_mx"),
...
def landsat_composites(year, start, end, roi, append_name, composites_only=False):
    ...
    def evi_(x):
        return x.expression(
            ...
            {"NIR": x.select("B5"), "RED": x.select("B4"), "BLUE": x.select("B2")},
        )

    def gi_(x):
        return x.expression(
            "NIR / GREEN", {"NIR": x.select("B5"), "GREEN": x.select("B3")}
        )

    bands_means = None
    lsSR_masked = landsat_masked(year, roi)
    if not composites_only:
        bands_means = ee.Image(
            lsSR_masked.filterDate(start, end)
            .map(
                lambda x: x.select(
    ...
    if append_name in ["m2", "m1", "gs"]:
        ndvi_mx = ee.Image(
            ...
            .map(lambda x: x.normalizedDifference(["B5", "B4"]))
            .max()
        ).rename("nd_max_{}".format(append_name))

        ndvi_mean = ee.Image(
            lsSR_masked.filterDate(start, end)
            .map(lambda x: x.normalizedDifference(["B5", "B4"]))
            .mean()
        ).rename("nd_mean_{}".format(append_name))
        ...
        ndvi = ee.Image(
            ...
            .map(lambda x: x.normalizedDifference(["B5", "B4"]))
            .max()
        ).rename("nd_{}".format(append_name))

    ndwi = ee.Image(
        lsSR_masked.filterDate(start, end)
        .map(lambda x: x.normalizedDifference(["B5", "B6"]))
        .max()
    ).rename("nw_{}".format(append_name))
    evi = ee.Image(
        lsSR_masked.filterDate(start, end).map(lambda x: evi_(x)).max()
    ).rename("evi_{}".format(append_name))
    gi = ee.Image(
        lsSR_masked.filterDate(start, end).map(lambda x: gi_(x)).max()
    ).rename("gi_{}".format(append_name))
        ...
        for k in list(scenes.keys()):
            ...
            if local_check:
                ...
                local_file = os.path.join(
                    local_check, "{}_{}.tif".format(f, month_number)
                )
