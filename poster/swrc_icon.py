...
def plot_swrc_icon(
    results_path, out_path, depth_key=None, show=False, transparent=True
):
    ...
    if psi_obs.size and theta_obs.size:
        ax.plot(
            theta_obs, psi_obs, "o", ms=6.0, mec="none", color="#1f77b4", alpha=0.95
        )
...
if __name__ == "__main__":
    ...
    candidates_ = [
        f
        for f in os.listdir(fits_dir_)
        if f.startswith(site_base_) and f.endswith(".json")
    ]
    results_path_ = os.path.join(fits_dir_, sorted(candidates_)[0])
    out_path_ = os.path.join(
        "poster", f"swrc_icon_{site_base_.lower().replace('-', '_')}.png"
    )
