...
def merge_data(fitted_results_dir, rosetta_param_file, inferred_param_file):
    ...
    fit_records = []
    fit_files = [
        os.path.join(fitted_results_dir, f)
        for f in os.listdir(fitted_results_dir)
        if f.endswith(".json")
    ]
    for jf in fit_files:
        with open(jf, "r") as f:
    ...
    for station_id, row in rosetta_df.iterrows():
        ...
        for level, params in params_by_level.items():
            if len(params) >= 4:
                ros_records.append(
                    {"station": station_id, "level": level, "rosetta_params": params}
                )
    ...
    pred_cols = [
        c for c in inferred_df.columns if c in ["theta_r", "theta_s", "alpha", "n"]
    ]
...
def compare_parameter_errors(merged_df):
    ...
    df = merged_df.dropna(
        subset=["fit_params", "rosetta_params", "inferred_params"]
    ).copy()
    ...
    df = pd.concat([df.reset_index(), fit_p, ros_p, inf_p], axis=1)
...
if __name__ == "__main__":
    ...
    fitted_dir_ = os.path.join(
        root_, "soil_potential_obs", "curve_fits", "mt_mesonet", "bayes"
    )
    rosetta_pq_ = os.path.join(
        root_, "rosetta", "mt_mesonet", "extracted_rosetta_points.parquet"
    )
    # inferred_pq_ = os.path.join(swap_root_, 'training', 'predictions', 'stations_predictions_nn.parquet')

    inferred_pq_ = os.path.join(
        swap_root_, "training", "predictions", "stations_predictions.parquet"
    )
