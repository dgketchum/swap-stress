...
class InverseTest(SWRC):
    def __init__(
        ...
        self._vg_model = (
            Model(self._inverse_log10_model)
            if self._inverse_log
            else Model(self._inverse_van_genuchten_model)
        )
    ...
    def _inverse_log10_model(theta, theta_r, theta_s, alpha, n):
        psi = InverseTest._inverse_van_genuchten_model(
            theta, theta_r, theta_s, alpha, n
        )
    ...
    def fit(self, report=True, method="nelder"):
        ...
        for depth, data_df in self.data_by_depth.items():
            ...
            try:
                y = (
                    d["suction"]
                    if not self._inverse_log
                    else np.log10(np.maximum(d["suction"].values, 1e-9))
                )
                w = None
...
def plot_inverse_vs_forward(station_key, reesh_csv_dir, forward_json_path, out_dir):
    ...
    if inv_res and inv_res.success:
        p = inv_res.params
        psi_pred = InverseTest._inverse_van_genuchten_model(
            theta_grid,
            p["theta_r"].value,
            p["theta_s"].value,
            p["alpha"].value,
    ...
    ax = axes[1]
    ax.plot(d_obs["theta"], d_obs["suction"], "o", ms=5, alpha=0.75, label="Observed")
    if params_fwd and all(
        k in params_fwd for k in ("theta_r", "theta_s", "alpha", "n")
    ):
...
if __name__ == "__main__":
    ...
    forward_json_ = os.path.join(
        root_, "curve_fits", "reesh", "nelder", f"{station_key_}.json"
    )
