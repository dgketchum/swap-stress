...
def run_xgb_training(f, levels=None):
    ...
    if has_rosetta:
        ...
        if targets:
            level_id = 'GSHP_VG_combined'
            print(f"\n--- Training combined XGB for GSHP with {len(df)} Samples ---")
            df_ = df.copy()
            if 'data_flag' in df_.columns:
                df_ = df_[df_['data_flag'] == 'good quality estimate']
                if 'data_flag' in feature_cols:
                    feature_cols.remove('data_flag')  # likely not present; safe guard

            data = df_[targets + feature_cols].copy()
            initial_len = len(data)
            for t in targets:
                data[data[t] <= -9999] = np.nan
            ...
            all_metrics[level_id] = metrics
            print(f"Metrics for combined XGB model GSHP: {metrics}")
...
if __name__ == '__main__':
    ...
    metrics_json_ = os.path.join(metrics_dst, f'RandomForest_combined_{timestamp_}.json')
    ...
    # all_metrics_xgb_ = run_xgb_training(f_, levels=None)
    #
    # timestamp_xgb_ = datetime.now().strftime('%Y%m%d_%H%M%S')
    # metrics_json_xgb_ = os.path.join(metrics_dir_, f'XGBoost{timestamp_xgb_}.json')
    #
