...
def build_gshp_training_table(
    ...
    if not features_path or not os.path.exists(features_path):
        raise ValueError(
            "features_path is required and must point to stations current_features.csv"
        )
    ...
    labels_df = (
        pd.read_csv(labels_csv, encoding="latin1")
        if labels_csv.endswith(".csv")
        else pd.read_parquet(labels_csv)
    )
    ...
    if "hzn_top" in labels_df.columns and "hzn_bot" in labels_df.columns:
        depth_series = (
            labels_df["hzn_top"].astype(float) + labels_df["hzn_bot"].astype(float)
        ) / 2.0
    ...
    final_df = ee_df.join(labels_df, how="left").dropna(
        subset=["theta_r", "theta_s", "alpha", "n"]
    )
    ...
    if embeddings and os.path.isdir(embeddings):
        ...
        if emb_files:
            rows = {}
            for fp in tqdm(
                emb_files, desc="Adding Embedding Features", total=len(emb_files)
            ):
    ...
    print(
        f"{len(final_df)} x {final_df.shape[1]} rows GSHP training samples written to {out_file}"
    )
...
if __name__ == "__main__":
    ...
    if run_gshp_workflow:
        ...
        labels_csv_ = os.path.join(
            gshp_directory_, "WRC_dataset_surya_et_al_2021_final_clean.csv"
        )
        ee_features_pqt_ = os.path.join(
        ...
        features_csv_ = os.path.join(
            root_, "soils", "swapstress", "training", "current_features.csv"
        )
