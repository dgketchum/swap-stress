...
def evaluate_site(
    ...
    metrics_all = evaluate_theta_vs_psi(
        df, targets=targets, mask=None, n_splits=n_splits
    )
    out = {"all": metrics_all}
    if use_drydown and "theta" in df.columns:
        mask = detect_drydown_mask(df)
        metrics_dd = evaluate_theta_vs_psi(
            df, targets=targets, mask=mask, n_splits=n_splits
        )
        ...
        if os.path.exists(data_fp) and not overwrite:
            ...
            try:
                ...
                if os.path.isdir(amf_root_or_file):
                    ...
                    amf_fp = (
                        amf_root_or_file if os.path.exists(amf_root_or_file) else None
                    )
                ...
                if amf_fp is None:
                    ...
                    if status == "ok":
                        ...
                        if df is not None and not df.empty:
                            ...
                            print(
                                f"wrote {sid}; {df.shape[1]} columns, {len(df)} days to {data_fp}"
                            )
                        else:
                            status = "missing"
                ...
                rec["n_psi_cols"] = int(
                    len(
                        [
                            c
                            for c in df.columns
                            if isinstance(c, str) and c.startswith("psi_cm_")
                        ]
                    )
                )
                counts = df.count()
            ...
            try:
                gdf = gpd.GeoDataFrame(
                    ...
                    geometry=gpd.points_from_xy(
                        summary_df.dropna(subset=["lon", "lat"])["lon"],
...
def run_regressions(
    ...
    for sid, fp in files:
        ...
        base_all = evaluate_theta_vs_psi(
            d_base, targets=tgt_list, mask=None, n_splits=n_splits
        )
        base_dd = evaluate_theta_vs_psi(
            d_base,
            targets=tgt_list,
            mask=detect_drydown_mask(d_base),
            n_splits=n_splits,
        )

        metrics_out: Dict[str, Dict] = {
            "theta": {"all": base_all, "drydown": base_dd},
            "replicates": {},
        }
        rep_cols = [
            c for c in df.columns if isinstance(c, str) and c.startswith("psi_cm_")
        ]
        for col in rep_cols:
            rep = col.split("__", 1)[1] if "__" in col else col
            d_rep = df[["theta"] + tgt_list].copy()
            d_rep["psi_cm"] = df[col]
            rep_all = evaluate_theta_vs_psi(
                d_rep, targets=tgt_list, mask=None, n_splits=n_splits
            )
            rep_dd = evaluate_theta_vs_psi(
                d_rep,
                targets=tgt_list,
                mask=detect_drydown_mask(d_rep),
                n_splits=n_splits,
            )
        ...
        if mv_enabled:
            ...
            for t in tgt_list:
                # MV with theta
                mv_theta_all = evaluate_multivariate(
                    df,
                    target=t,
                    predictors=["theta", "pet", "pr"],
                    mask=None,
                    n_splits=n_splits,
                )
                mv_theta_dd = evaluate_multivariate(
                    df,
                    target=t,
                    predictors=["theta", "pet", "pr"],
                    mask=mask_dd,
                ...
                metrics_out["mv"]["theta"][t] = {
                    "all": mv_theta_all,
                    "drydown": mv_theta_dd,
                }
                # Lagged with theta (prototype set of lags)
                lag_theta_all = evaluate_lagged(
                    df,
                    target=t,
                    base_predictor="theta",
                    covariates=["pet", "pr"],
                    lags=(0, 3, 7, 14),
                    mask=None,
                    n_splits=n_splits,
                )
                lag_theta_dd = evaluate_lagged(
                    df,
                    target=t,
                    base_predictor="theta",
                    covariates=["pet", "pr"],
                ...
                metrics_out["lagged"]["theta"][t] = {
                    "all": lag_theta_all,
                    "drydown": lag_theta_dd,
                }

                # MV and lagged for psi replicates
                for col in rep_cols:
                    rep = col.split("__", 1)[1] if "__" in col else col
                    mv_rep_all = evaluate_multivariate(
                        df.rename(columns={col: "psi_cm"}),
                        target=t,
                        predictors=["psi_cm", "pet", "pr"],
                        mask=None,
                        n_splits=n_splits,
                    )
                    mv_rep_dd = evaluate_multivariate(
                        df.rename(columns={col: "psi_cm"}),
                        target=t,
                        predictors=["psi_cm", "pet", "pr"],
                        mask=mask_dd,
                    ...
                    metrics_out["mv"]["replicates"].setdefault(rep, {})[t] = {
                        "all": mv_rep_all,
                        "drydown": mv_rep_dd,
                    }

                    lag_rep_all = evaluate_lagged(
                        df.rename(columns={col: "psi_cm"}),
                        target=t,
                        base_predictor="psi_cm",
                        covariates=["pet", "pr"],
                        lags=(0, 3, 7, 14),
                        mask=None,
                        n_splits=n_splits,
                    )
                    lag_rep_dd = evaluate_lagged(
                        df.rename(columns={col: "psi_cm"}),
                        target=t,
                        base_predictor="psi_cm",
                        covariates=["pet", "pr"],
                    ...
                    metrics_out["lagged"]["replicates"].setdefault(rep, {})[t] = {
                        "all": lag_rep_all,
                        "drydown": lag_rep_dd,
                    }
        ...
        for t in tgt_list:
            ...
            titles = [f"{sid}: {t} ~ theta"] + [
                f"{sid}: {t} ~ psi ({c.split('__', 1)[1]})" for c in rep_cols
            ]
            out_png = os.path.join(fig_dir, f"{sid}_{t.lower()}.png")
            plot_scatter_with_fit(
                df, x_cols=x_cols, y_col=t, titles=titles, out_path=out_png
            )
        ...
        for t in tgt_list:
            ...
            for scope in ["all", "drydown"]:
                row[f"{scope}_r2"] = (
                    float(base_all[t]["theta"]["r2_mean"])
                    if scope == "all"
                    else float(base_dd[t]["theta"]["r2_mean"])
                )
                row[f"{scope}_rmse"] = (
                    float(base_all[t]["theta"]["rmse_mean"])
                    if scope == "all"
                    else float(base_dd[t]["theta"]["rmse_mean"])
                )
                row[f"{scope}_folds"] = (
                    int(base_all[t]["theta"]["folds"])
                    if scope == "all"
                    else int(base_dd[t]["theta"]["folds"])
                )
            rows.append(row)
            ...
            if "lagged" in metrics_out:
                ...
                for scope in ["all", "drydown"]:
                    ...
                    best_lag = max(
                        lag_metrics.items(),
                        key=lambda kv: (
                            kv[1].get("r2_mean") if isinstance(kv[1], dict) else -np.inf
                        ),
                    )[0]
...
if __name__ == "__main__":
    ...
    coords_by_id_ = {
        str(r["site_id"]): (float(r.geometry.x), float(r.geometry.y))
