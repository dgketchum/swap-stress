...
def _dyn_worker(params):
    ...
    for s0, _ in keep:
        out.append(
            {"file": fp, "gm_file": gmp, "start": int(s0), "stop": int(s0 + window_len)}
        )
    return out


def _build_dynamic_windows(
    files, gridmet_dir, window_len, stride_days=7, topk_overall=None, num_workers=24
):
    windows = []
    if not files:
        return windows
    with ProcessPoolExecutor(max_workers=num_workers) as ex:
        futures = {
            ex.submit(
                _dyn_worker, (fp, gridmet_dir, window_len, stride_days, topk_overall)
            ): fp
            for fp in files
        }
        for fut in tqdm(
...
def run_training(
    ...
    window_len=730,
    stride=550,
    static_features_pqt=None,
    static_id_col=None,
    static_cols=None,
    use_dynamic_windows=False,
    dyn_stride_days=7,
    dyn_topk_per_year=1,
):
    files = (
        ...
        else sorted(glob(os.path.join(data_dir, "*.parquet")))
    )
    ...
    if static_features_pqt is not None and os.path.exists(static_features_pqt):
        ...
        sdf = (sdf - means) / stds
        static_map = {
            str(idx): row.values.astype("float32") for idx, row in sdf.iterrows()
        }
    ...
    if gridmet_dir:
        gm_exist = {
            vid
            for vid in vwc_ids
            if os.path.exists(os.path.join(gridmet_dir, f"{vid}.parquet"))
        }
    else:
    ...
    g0 = (
        pd.read_parquet(gm0)
        if (gm0 and os.path.exists(gm0))
        else pd.DataFrame(index=v0.index)
    )
    ...
    if use_dynamic_windows:
        train_windows = _build_dynamic_windows(
            train_files,
            gridmet_dir,
            window_len,
            stride_days=dyn_stride_days,
            topk_overall=10,
            num_workers=24,
        )
        val_windows = _build_dynamic_windows(
            val_files,
            gridmet_dir,
            window_len,
            stride_days=dyn_stride_days,
    ...
    train_ds = CombinedVwcDataset(
        train_windows, zscore=True, mask_mode="mixed", static_map=static_map
    )
    val_ds = CombinedVwcDataset(
        val_windows, zscore=True, mask_mode="mixed", static_map=static_map
    )
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=ckpt_dir,
        filename=f"mae-both-{datetime.now().strftime('%Y%m%d')}-"
...
if __name__ == "__main__":
    ...
    run_training(
        ...
        epochs=epochs_,
        mask_ratio=mask_ratio_,
        num_workers=workers_,
        gridmet_dir=gridmet_dir_,
        static_cols=static_cols_,
        static_features_pqt=static_features_pqt_,
        static_id_col=static_id_col_,
        window_len=180,
