...
def stack_bands_climatology(
    roi, start_yr=1991, end_yr=2020, subselection=None, region="conus"
):
    ...
    for name, start_doy, end_doy in periods:
        landsat_stats = landsat_composites(
            start_yr, end_yr, start_doy, end_doy, roi, name
        )
    ...
    if region == "conus":
        ...
        gridmet_coll = ee.ImageCollection("IDAHO_EPSCOR/GRIDMET").filterDate(
        ...
        for name, start_doy, end_doy in seasons:
            if start_doy > end_doy:
                ...
                season_filter = ee.Filter.calendarRange(
            ...
            climate_stats = seasonal_gridmet.select(
                ["pr", "eto", "tmmn", "tmmx"]
            ).reduce(reducers)

            mean_temp = (
                climate_stats.select("tmmn_mean")
                .add(climate_stats.select("tmmx_mean"))
                .divide(2)
                .rename("tmean_mean")
            )
            ...
            new_names = climate_stats.bandNames().map(
                lambda b: ee.String(b).cat("_").cat(name)
            )
    ...
    s1_stdDev = s1_coll_with_ratio.reduce(ee.Reducer.stdDev()).rename(
    ...
    smap_l3_am_clim = smap_l3_coll.select(["vegetation_water_content_am"]).reduce(
        ee.Reducer.mean().combine(ee.Reducer.stdDev(), "", True)
    )
    smap_l3_pm_clim = smap_l3_coll.select(["vegetation_water_content_pm"]).reduce(
        ee.Reducer.mean().combine(ee.Reducer.stdDev(), "", True)
    )
    ...
    smap_l4_clim = smap_l4_coll.reduce(
        ee.Reducer.mean().combine(ee.Reducer.stdDev(), "", True)
    )
    ...
    hihydro_bands = ee.Image.cat(
        [
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/ksat")
            .mean()
            .rename("hhs_ksat"),
            ee.ImageCollection(
                "projects/sat-io/open-datasets/HiHydroSoilv2_0/sat-field"
            )
            .mean()
            .rename("hhs_sat_field"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/N")
            .mean()
            .rename("hhs_n"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/alpha")
            .mean()
            .rename("hhs_alpha"),
            ee.ImageCollection(
                "projects/sat-io/open-datasets/HiHydroSoilv2_0/crit-wilt"
            )
            .mean()
            .rename("hhs_crit_wilt"),
            ee.ImageCollection(
                "projects/sat-io/open-datasets/HiHydroSoilv2_0/field-crit"
            )
            .mean()
            .rename("hhs_field_crit"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/ormc")
            .mean()
            .rename("hhs_ormc"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/stc")
            .mode()
            .rename("hhs_stc"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/wcavail")
            .mean()
            .rename("hhs_wcavail"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/wcpf2")
            .mean()
            .rename("hhs_wcpf2"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/wcpf3")
            .mean()
            .rename("hhs_wcpf3"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/wcpf4-2")
            .mean()
            .rename("hhs_wcpf4_2"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/wcres")
            .mean()
            .rename("hhs_wcres"),
            ee.ImageCollection("projects/sat-io/open-datasets/HiHydroSoilv2_0/wcsat")
            .mean()
            .rename("hhs_wcsat"),
        ]
    )
    ...
    if region == "conus":
        static_bands.extend(
            [nlcd, cdl_bands, ssurgo, prism, additional_terrain, us_lith, polaris]
        )
