...
_INDEX_LABELS = {"nd": "NDVI", "nw": "NDWI", "evi": "EVI", "gi": "Green Index"}
...
_AMSR_VOD = {
    "vod_asc": "AMSR VOD 10.7 GHz (ascending)",
    "vod_desc": "AMSR VOD 10.7 GHz (descending)",
}
...
def label_feature(n):
    ...
    m = re.match(
        r"^(pr|eto|tmmn|tmmx|tmean)_(mean|stdDev)_(winter|spring|summer|autumn)$", x
    )
    if m:
    ...
    m = re.match(
        r"^(sm_surface|sm_rootzone|sm_profile|surface_temp|leaf_area_index)_(mean|stdDev)$",
        x,
    )
    if m:
        v, stat = m.groups()
    ...
    m = re.match(
        r"^(vod_asc|vod_desc)_(mean|stddev)_(winter|spring|summer|autumn|annual)$", x
    )
    if m:
        base, stat, period = m.groups()
        base_lbl = _AMSR_VOD.get(base, base)
        stat_lbl = {"mean": "mean", "stddev": "std dev"}.get(stat, stat)
        lbl = f"{base_lbl} {stat_lbl} ({period})"
        return lbl

    # SoilGrids depth-resolved layers, e.g., clay_100-200cm_mean
    m = re.match(
        r"^(bdod|cec|cfvo|clay|sand|silt|nitrogen|phh2o|soc|ocd|ocs)_(\d+-\d+cm)_(mean|stdDev|sd)$",
        x,
    )
    if m:
...
def build_feature_label_map(features_csv):
    ...
    if len(emb) > 0 or len(bn) >= 32:
        ...
        if "b1" in feats and "b1_1" in feats:
            special["b1"] = (
                "Terrain wetness index"  # likely error: ambiguous origin of b1
            )
