...
def standardize_gshp(df, depth_col=None):
    ...
    keep += [c for c in ('profile_id', 'SWCC_classes') if c in d.columns]
    return d[keep]
...
def write_standardized_gshp(soil_csv_path, out_dir):
    ...
    for pid, r in tqdm(std.groupby('profile_id'), total=len(std.groupby('profile_id'))):
        ...
        r[['suction', 'theta', 'depth', 'SWCC_classes']].to_csv(out_path, index=False)
        stations += 1
...
if __name__ == '__main__':
    ...
    if run_gshp:
        ...
        write_standardized_gshp(soil_csv_path_, out_dir_)
