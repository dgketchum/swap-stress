...
def _load_sources(include_rosetta=True):
    ...
    mesonet_shp = os.path.join(
        soils_, "soil_potential_obs", "mt_mesonet", "station_metadata_clean_mgrs.shp"
    )
    gshp_shp = os.path.join(
        soils_, "soil_potential_obs", "gshp", "wrc_aggregated_mgrs.shp"
    )
...
def plot_training_data_map(out_path, figsize=(10, 6), dpi=300, include_rosetta=True):
    ...
    fig, (ax_g, ax_c) = plt.subplots(
        2, 1, figsize=(figsize[0], figsize[1] * 2.2), dpi=dpi
    )
    ...
    reesh_in = gpd.sjoin(
        reesh, conus[["geometry"]], predicate="intersects", how="inner"
    )
    mesonet_in = gpd.sjoin(
        mesonet, conus[["geometry"]], predicate="intersects", how="inner"
    )
    ...
    if not reesh_g.empty:
        reesh_g.plot(
            ax=ax_g, markersize=18, marker="^", color="#d62728", linewidth=0.2, zorder=3
        )
    if not mesonet_g.empty:
        mesonet_g.plot(
            ax=ax_g, markersize=16, marker="o", color="#1f77b4", linewidth=0.2, zorder=3
        )
    if not gshp_g.empty:
        gshp_g.plot(
            ax=ax_g, markersize=16, marker="s", color="#2ca02c", linewidth=0.2, zorder=3
        )
    ...
    rosetta_c = (
        rosetta.cx[xmin:xmax, ymin:ymax]
        if include_rosetta and rosetta is not None
        else None
    )
    if not reesh_c.empty:
        reesh_c.plot(
            ax=ax_c, markersize=24, marker="^", color="#d62728", linewidth=0.2, zorder=3
        )
    if not mesonet_c.empty:
        mesonet_c.plot(
            ax=ax_c, markersize=16, marker="o", color="#1f77b4", linewidth=0.2, zorder=3
        )
    if not gshp_c.empty:
        gshp_c.plot(
            ax=ax_c, markersize=16, marker="s", color="#2ca02c", linewidth=0.2, zorder=3
        )
    if rosetta_c is not None and not rosetta_c.empty:
        rosetta_c.plot(
            ax=ax_c, markersize=3, marker="x", color="#ff7f0e", linewidth=0.2, zorder=3
        )
