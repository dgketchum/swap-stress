        ...
        try:
            ...
            if "layer_id" in df_in.columns:
                ...
                tmp = tempfile.NamedTemporaryFile(
                    prefix="rfit_samples_", suffix=".csv", delete=False
                )
                tmp.close()
...
if __name__ == "__main__":
    ...
    if run_rfit and ptf_rds:
        if not os.path.exists(ptf_rds):
            print(
                f"PTF model RDS not found at {ptf_rds}; skipping fit_new_samples.R call."
            )
            ...
            rc = run_fit_new_samples(
                out_csv_rfit, ptf_rds, fit_out_dir, rscript_path=rscript_path
            )
            if rc != 0:
