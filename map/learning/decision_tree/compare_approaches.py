...
from map.learning.decision_tree.train_decision_trees import (
    filter_feature_groups, FEATURE_GROUPS, _EMBEDDING_PATTERNS
)
...
class VGExperimentConfig:
    ...
    log_transform_params: Optional[List[str]] = None  # VG params to predict on log10 scale
    ...
    def __post_init__(self):
        ...
        if self.log_transform_params:
            ...
            if invalid:
                raise ValueError(f"Invalid log_transform_params: {invalid}. Must be subset of {valid_params}")
...
DEFAULT_VG_EXPERIMENTS = [
    ...
    VGExperimentConfig(name='vg_no_soilgrids', depth_handling='feature',
                       exclude_feature_groups=['soilgrids', 'fao']),
    VGExperimentConfig(name='vg_no_embeddings', depth_handling='feature',
                       exclude_feature_groups=['embeddings']),
    VGExperimentConfig(name='vg_no_polaris', depth_handling='feature',
                       exclude_feature_groups=['polaris']),
    VGExperimentConfig(name='vg_minimal', depth_handling='feature',
    ...
    VGExperimentConfig(name='vg_base_data', depth_handling='feature',
                       exclude_feature_groups=['embeddings', 'polaris', 'smap']),
    VGExperimentConfig(name='vg_base_data_no_soils', depth_handling='feature',
    ...
    VGExperimentConfig(name='vg_log_alpha_n', depth_handling='feature',
                       log_transform_params=['alpha', 'n']),
    VGExperimentConfig(name='vg_log_alpha', depth_handling='feature',
...
DEFAULT_DIRECT_EXPERIMENTS = [
    ...
    DirectExperimentConfig(name='direct_base_data',
                           exclude_feature_groups=['embeddings', 'polaris', 'smap']),
    DirectExperimentConfig(name='direct_base_data_no_soils',
                           exclude_feature_groups=['embeddings', 'polaris', 'smap', 'soilgrids', 'fao']),
]
...
NON_FEATURE_COLS = {
    ...
    'theta', 'suction_cm', 'log10_suction_cm',
    # Identifiers
    'sample_id', 'profile_id', 'station', 'site_id', 'obs_id',
    # Source/metadata
    'source', 'data_flag', 'SWCC_class', 'SWCC_classes', 'data_ct',
    # Network metadata
    'nwsli_id', 'network', 'mesowest_i', 'obs_ct',
    # Spatial identifiers
    'MGRS_TILE', 'lat', 'lon', 'latitude', 'longitude',
    # Depth (handle separately)
    'rosetta_level', 'depth_cm', 'depth',
}


def filter_complete_samples(df: pd.DataFrame, required_feature_count: int = 200) -> pd.DataFrame:
    """
    ...
    if 'lat' in df.columns and 'lon' in df.columns:
        ...
        print(f"  Filtered to samples with lat/lon: {len(df)}/{initial_count} "
...
def audit_dataset(
    ...
    if 'source' not in df.columns:
        ...
        return {'counts_by_source': {}, 'missingness_by_source': pd.DataFrame(), 'blocking_features': []}
...
def filter_blocking_features(
    ...
    if n_removed > 0:
        print(f"  Removed {n_removed} blocking features: {blocking_features[:5]}{'...' if len(blocking_features) > 5 else ''}")
    return filtered
...
def van_genuchten(theta: np.ndarray, theta_r: np.ndarray, theta_s: np.ndarray,
                  alpha: np.ndarray, n: np.ndarray) -> np.ndarray:
    """
...
def get_feature_cols_for_config(
    ...
    if config.exclude_feature_groups:
        feature_cols = filter_feature_groups(feature_cols, config.exclude_feature_groups)
    ...
    if config.depth_handling == 'per_level':
        ...
        for level in levels:
            ...
            models, preprocessor = train_vg_model(
                level_df, feature_cols, config.n_estimators, random_state,
                log_transform_params=log_params
            )
            level_models[level] = models
            level_preprocessors[level] = preprocessor

        return {'level_models': level_models, 'level_preprocessors': level_preprocessors,
                'levels': levels, 'log_transform_params': log_params}
    else:
        # Train single set of models across all levels
        models, preprocessor = train_vg_model(
            train_df, feature_cols, config.n_estimators, random_state,
            log_transform_params=log_params
        )
        return {'models': models, 'preprocessor': preprocessor, 'log_transform_params': log_params}
...
def compute_metrics_by_site(
    ...
    if len(per_site_df) > 0:
        ...
        site_weighted = {
            ...
            'mean_mae': np.nan, 'median_mae': np.nan,
            'n_sites': 0,
        }
...
def compute_stratified_metrics(
    ...
    for low, high, name in theta_bins:
        ...
        if mask.sum() > 0:
            ...
            direct_metrics = compute_metrics(y_true[mask], y_pred_direct[mask], linear=linear)
            results.append({
...
def plot_comparison(
    ...
    ax = axes[0, 0]
    ax.scatter(y_true[valid_vg], y_pred_vg[valid_vg], alpha=0.1, s=1)
    ax.plot([0, 7], [0, 7], 'r--', lw=2)
    ...
    ax = axes[0, 1]
    ax.scatter(y_true[valid_direct], y_pred_direct[valid_direct], alpha=0.1, s=1)
    ax.plot([0, 7], [0, 7], 'r--', lw=2)
    ax.set_xlabel('Observed log10(ψ)')
    ax.set_ylabel('Predicted log10(ψ)')
    ...
    ax.hist(residuals_vg, bins=50, alpha=0.5, label=f'VG (σ={np.std(residuals_vg):.2f})', density=True)
    ax.hist(residuals_direct, bins=50, alpha=0.5, label=f'Direct (σ={np.std(residuals_direct):.2f})', density=True)
    ...
    ax.scatter(theta[valid_vg], error_vg[valid_vg], alpha=0.1, s=1)
    ax.set_xlabel('θ (VWC)')
    ...
    ax.scatter(theta[valid_direct], error_direct[valid_direct], alpha=0.1, s=1)
    ax.set_xlabel('θ (VWC)')
    ax.set_ylabel('|Error| log10(ψ)')
    ...
    ax.scatter(y_pred_vg[both_valid], y_pred_direct[both_valid], alpha=0.1, s=1, c=theta[both_valid], cmap='viridis')
    ax.plot([0, 7], [0, 7], 'r--', lw=2)
...
def plot_retention_curves(
    ...
    for idx, sample_id in enumerate(good_samples):
        ...
        y_direct = y_pred_direct[mask.values] if hasattr(mask, 'values') else y_pred_direct[mask]
...
def plot_vg_model_comparison(
    ...
    sorted_models = sorted(vg_results.items(), key=lambda x: x[1]['suction_r2'], reverse=True)
    ...
    ax.axhline(y=direct_r2, color='red', linestyle='--', lw=2,
...
def run_systematic_comparison(
    ...
    if 'source' in obs_df.columns:
        ...
        if blocking_features:
            print(f"   Blocking features (100% missing for some source): {blocking_features[:10]}{'...' if len(blocking_features) > 10 else ''}")
            if drop_blocking_features:
                common_base_features = filter_blocking_features(common_base_features, blocking_features)
                print(f"   Features after removing blocking features: {len(common_base_features)}")
            else:
    ...
    train_sites, test_sites = create_site_split(vg_df, 'sample_id', test_size, random_state)
    ...
    for config in experiments:
        ...
        feature_cols = [f for f in feature_cols if f in common_base_features or
                        f in ['rosetta_level', 'depth_cm']]
        # Further filter to columns that actually exist
        feature_cols = [f for f in feature_cols if f in vg_train.columns and f in obs_test.columns]
        ...
        if config.depth_handling == 'per_level':
            y_pred = predict_vg_per_level(
                model_result['level_models'], obs_test_clean, feature_cols,
                log_transform_params=log_params,
                level_preprocessors=model_result.get('level_preprocessors')
            )
        else:
            y_pred = predict_vg_approach(
                model_result['models'], obs_test_clean, feature_cols,
                log_transform_params=log_params,
                    ...
                    if len(X_vg_test_raw) > 0:
                        ...
                        if param in log_params_set:
                            ...
                            vg_param_metrics[param] = {
                                ...
                                'rmse': float(np.sqrt(mean_squared_error(y_vg_true_log, y_vg_pred))),
                                'scale': 'log10',
                            }
                        else:
                            vg_param_metrics[param] = {
                                'r2': float(r2_score(y_vg_true, y_vg_pred)),
                                'rmse': float(np.sqrt(mean_squared_error(y_vg_true, y_vg_pred))),
                                'scale': 'natural',
                            }
    ...
    for config in direct_experiments:
        ...
        # Apply feature group exclusions
        if config.exclude_feature_groups:
            direct_features = filter_feature_groups(direct_features, config.exclude_feature_groups)
        ...
        direct_models[config.name] = (direct_model, direct_features, direct_preprocessor)

        y_pred_direct = predict_direct_approach(
            direct_model, obs_test_clean, direct_features, preprocessor=direct_preprocessor
        )
    ...
    best_direct_name = max(direct_results.keys(), key=lambda k: direct_results[k]['suction_r2'])
    ...
    print("-" * 50)
    for name, res in sorted(vg_results.items(), key=lambda x: x[1]['suction_r2'], reverse=True):
        marker = " *BEST VG" if name == best_vg_name else ""
        print(f"{name:<30} {res['suction_r2']:<10.4f} {res['suction_rmse']:<10.4f}{marker}")
    ...
    print("-" * 60)
    for name, res in sorted(direct_results.items(), key=lambda x: x[1]['suction_r2'], reverse=True):
        marker = " *BEST" if name == best_direct_name else ""
        print(f"{name:<30} {res['suction_r2']:<10.4f} {res['suction_rmse']:<10.4f} {res['n_features']:<10}{marker}")
    ...
    if best_vg['suction_r2'] > best_direct['suction_r2']:
        improvement = (best_vg['suction_r2'] - best_direct['suction_r2']) / best_direct['suction_r2'] * 100
        print(f"\nBest VG model ({best_vg_name}) beats best direct model!")
        print(f"R² improvement: {improvement:.1f}%")
    else:
        improvement = (best_direct['suction_r2'] - best_vg['suction_r2']) / abs(best_vg['suction_r2']) * 100
    ...
    plot_vg_model_comparison(
        vg_results, best_direct['suction_r2'],
    ...
    best_features = [f for f in best_features if f in common_base_features or
                     f in ['rosetta_level', 'depth_cm']]

    vg_train_clean = vg_train.dropna(subset=[f for f in best_features if f in vg_train.columns])
    best_model_result = train_vg_with_depth_handling(
        vg_train_clean, best_features, best_config, random_state
    )

    required_cols = [f for f in best_features if f in obs_test.columns] + ['theta', 'log10_suction_cm']
    if 'rosetta_level' not in required_cols and best_config.depth_handling == 'per_level':
        required_cols.append('rosetta_level')
    obs_test_final = obs_test.dropna(subset=[c for c in required_cols if c in obs_test.columns])
    ...
    if best_config.depth_handling == 'per_level':
        y_pred_best_vg = predict_vg_per_level(
            best_model_result['level_models'], obs_test_final, best_features,
        ...
        y_pred_best_vg = predict_vg_approach(
            best_model_result['models'], obs_test_final, best_features,
            log_transform_params=best_log_params,
    ...
    plot_comparison(
        y_true_final, y_pred_best_vg, y_pred_direct_final, theta_final,
        os.path.join(output_dir, 'systematic_comparison.png')
    )

    # Compute stratified metrics
    print("\n6. Computing stratified metrics...")
    depth_final = obs_test_final['rosetta_level'].values if 'rosetta_level' in obs_test_final.columns else np.zeros(len(obs_test_final))
    stratified = compute_stratified_metrics(
        y_true_final, y_pred_best_vg, y_pred_direct_final, theta_final, depth_final,
        linear=linear
    )
    ...
    if 'source' in obs_test_final.columns:
        ...
        # Combine and save
        source_metrics_all = pd.concat([source_metrics_vg, source_metrics_direct], ignore_index=True)
        ...
        for source in source_metrics_vg['source'].unique():
            vg_r2 = source_metrics_vg[source_metrics_vg['source'] == source]['r2'].values[0]
            direct_r2 = source_metrics_direct[source_metrics_direct['source'] == source]['r2'].values[0]
            vg_n = source_metrics_vg[source_metrics_vg['source'] == source]['n'].values[0]
    ...
    # Combine and save
    site_metrics_all = pd.concat([site_metrics_vg, site_metrics_direct], ignore_index=True)
    ...
    print(f"      VG:     mean R²={site_weighted_vg['mean_r2']:.4f}, median R²={site_weighted_vg['median_r2']:.4f} ({site_weighted_vg['n_sites']} sites)")
    print(f"      Direct: mean R²={site_weighted_direct['mean_r2']:.4f}, median R²={site_weighted_direct['median_r2']:.4f} ({site_weighted_direct['n_sites']} sites)")
    ...
    print(f"   Best VG violation rate: {mono_vg['violation_rate']:.1%} "
          f"({mono_vg['violations']}/{mono_vg['total_samples']} samples)")
    print(f"   Direct violation rate: {mono_direct['violation_rate']:.1%} "
    ...
    results = {
        ...
        'comparison': {
            'winner': 'vg' if best_vg['suction_r2'] > best_direct['suction_r2'] else 'direct',
        ...
        'config': {
            ...
            'blocking_features_removed': blocking_features if drop_blocking_features else [],
        },
        'stratified_metrics': stratified.to_dict('records'),
        'source_metrics': {
            'vg': source_metrics_vg.to_dict('records') if source_metrics_vg is not None else [],
            'direct': source_metrics_direct.to_dict('records') if source_metrics_direct is not None else [],
        },
    ...
    # Save VG predictions
    vg_cols = ['sample_id', 'source', 'rosetta_level', 'depth_cm', 'theta',
               'log10_suction_cm', 'log10_suction_pred_vg', 'suction_cm_pred_vg']
    ...
    direct_cols = ['sample_id', 'source', 'rosetta_level', 'depth_cm', 'theta',
                   'log10_suction_cm', 'log10_suction_pred_direct', 'suction_cm_pred_direct']
...
def run_comparison(
    ...
    vg_train, vg_test = grouped_train_test_split(vg_df, 'sample_id', test_size, random_state)
    obs_train, obs_test = grouped_train_test_split(obs_df, 'sample_id', test_size, random_state)
    ...
    vg_models, vg_preprocessor = train_vg_model(vg_train, common_features, n_estimators, random_state)

    print("\n4. Training direct model...")
    direct_model, direct_preprocessor = train_direct_model(obs_train, common_features, n_estimators, random_state)

    # Generate predictions on test observations
    print("\n5. Generating predictions...")
    y_true = obs_test['log10_suction_cm'].values
    theta = obs_test['theta'].values
    depth = obs_test['rosetta_level'].values if 'rosetta_level' in obs_test.columns else np.zeros(len(obs_test))

    y_pred_vg = predict_vg_approach(vg_models, obs_test, common_features, preprocessor=vg_preprocessor)
    y_pred_direct = predict_direct_approach(direct_model, obs_test, common_features, preprocessor=direct_preprocessor)
    ...
    print("=" * 60)
    stratified = compute_stratified_metrics(y_true, y_pred_vg, y_pred_direct, theta, depth)
    print(stratified.to_string(index=False))
    ...
    print(f"VG violation rate: {mono_vg['violation_rate']:.1%} ({mono_vg['violations']}/{mono_vg['total_samples']} samples)")
    print(f"Direct violation rate: {mono_direct['violation_rate']:.1%} ({mono_direct['violations']}/{mono_direct['total_samples']} samples)")

    # Generate plots
    print("\n7. Generating plots...")
    plot_comparison(
        y_true, y_pred_vg, y_pred_direct, theta,
        os.path.join(output_dir, 'comparison_scatter.png')
    )

    plot_retention_curves(
        obs_test, y_pred_vg, y_pred_direct,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--n-estimators', type=int, default=250,
    ...
    parser.add_argument(
        '--drop-source-missing-features', action='store_true', default=True,
        dest='drop_blocking_features',
        help='Remove features that are 100%% missing for any source (default: ON). '
             'This prevents silent data dropping from sources with different feature availability.'
    )
    parser.add_argument(
        '--no-drop-source-missing-features', action='store_false',
        dest='drop_blocking_features',
    ...
    data_root = os.path.join(home, 'data', 'IrrigationGIS', 'soils', 'swapstress', 'training')
    ...
    if args.systematic:
        ...
        if args.experiments:
            ...
            direct_experiments = [e for e in DEFAULT_DIRECT_EXPERIMENTS if e.name in requested]

            found_names = [e.name for e in vg_experiments] + [e.name for e in direct_experiments]
            missing = set(requested) - set(found_names)
            if missing:
                print(f"Warning: experiments not found: {missing}")
                print(f"Available VG experiments: {[e.name for e in DEFAULT_VG_EXPERIMENTS]}")
                print(f"Available direct experiments: {[e.name for e in DEFAULT_DIRECT_EXPERIMENTS]}")

            if not vg_experiments and not direct_experiments:
                print("Error: No valid experiments selected")
                exit(1)

            print(f"Running selected experiments: VG={[e.name for e in vg_experiments]}, "
