...
def _load_reesh_swrc_params():
    ...
    try:
        df = extract_station_fit_params(
            results_dir, networks=("reesh",), fit_method="bayes"
        )
    except Exception:
        return None
    ...
    grouped = (
        df.groupby(["station", "depth_int"], dropna=False)[present].mean().reset_index()
    )
    return grouped


def _load_reesh_points(
    reesh_dir, property_column=None, property_key=None, swrc_params=None
):
    ...
    for fname in sorted(os.listdir(reesh_dir)):
        ...
        for _, row in df[cols].iterrows():
            ...
            if (
                property_key in SWRC_KEYS
                and swrc_map is not None
                and depth_col
                and site_col
            ):
                try:
...
def _load_gshp_points(gshp_file, property_column=None, property_key=None):
    ...
    if depth_cols.issubset(df.columns):
        ...
        if (
            property_column
            and property_column in df.columns
            and property_column not in cols
        ):
            cols.append(property_column)
        sub = df[cols].copy()
        ...
        grouped = sub.groupby(group_cols, dropna=False).agg(agg_spec).reset_index()
        source = grouped
    else:
...
def plot_texture_triangle(out_path, reesh_dir=None, gshp_file=None, property_key=None):
    ...
    if reesh_dir is None or gshp_file is None:
        ...
        if gshp_file is None:
            gshp_file = os.path.join(
                base, "gshp", "WRC_dataset_surya_et_al_2021_final.csv"
            )
    ...
    if use_color and (reesh_points or gshp_points):
        base_label = PARAM_SYMBOLS.get(
            property_key, property_key.replace("_", " ").title()
        )
        if property_key in {"alpha", "n"}:
