...
def build_station_training_table(
    ...
    _deprecation_warning()
    params_df = extract_station_fit_params(
        results_dir, include_subdirs, fit_method=fitting_method
    )
    if params_df.empty:
        return params_df
    ...
    merged = params_df.merge(
        ee_df, on="station", how="left", validate="m:1", suffixes=("", "_ee")
    )

    missing = merged["station"][
        merged[ee_df.columns.difference(["station"])].isna().all(axis=1)
    ].unique()
    if len(missing):
    ...
    if embeddings:
        ...
        for k, d in embeddings.items():
            ...
            for fp in emb_files:
                sid = (
                    os.path.splitext(os.path.basename(fp))[0].lower().replace("_", "-")
                )
                try:
    ...
    if out_file:
        ...
        print(
            f"Saving station training table to {out_file} {len(merged)} samples by {merged.shape[1]} rows"
        )
    ...
    if features_csv:
        ...
        print(
            f"Saved current features list to {features_csv}, with {len(features)} features"
        )
...
if __name__ == "__main__":
    ...
    if run_stations_workflow:
        ...
        ee_stations_pqt_ = os.path.join(
            root_, "swapstress", "training", "stations_ee_data_250m.parquet"
        )
        out_file_ = os.path.join(
            root_, "swapstress", "training", "stations_training_table_250m.parquet"
        )

        include_ = ("mt_mesonet", "reesh")
        features_csv_ = os.path.join(
            root_, "swapstress", "training", "current_features.csv"
        )
        ...
        build_station_training_table(
            ee_stations_pqt_,
            results_dir_,
            out_file_,
            include_subdirs=include_,
            embeddings=embeddings_map_,
