...
def plot_mesonet_swp_vs_vwc_with_violin(
    swrc_csv_path, vwc_parquet_path, save_path=None, show=False, station_name=None
):
    ...
    vwc_cols = [
        c
        for c in ts.columns
        if isinstance(c, str) and c.startswith("soil_vwc_") and not c.endswith("_units")
    ]
    ...
    depth_to_color = {
        int(round(float(d))): c for (d, _), c in zip(groups, colors) if pd.notna(d)
    }
    depth_cols = [
        (dd, f"soil_vwc_{dd:04d}")
        for dd in swrc_depths
        if f"soil_vwc_{dd:04d}" in ts.columns
    ]
    ...
    title_full = (
        f"{title_base} — {station_name}"
        if station_name
        else f"{title_base} — {station_code}"
    )
    ...
    for h, label in zip(handles, labels):
        ...
        )  # likely error: legend label missing depth digits
        pairs.append((d, h, label))
    pairs.sort(key=lambda x: x[0])
    handles_s = [h for _, h, _ in pairs]
    labels_s = [label for _, _, label in pairs]
    ax.legend(
        handles_s, labels_s, title="Depth", fontsize=9, title_fontsize=9, frameon=False
    )
...
def plot_mesonet_dirs_swp_vs_vwc_with_violin(
    swrc_dir, vwc_dir, out_dir, show=False, meta_csv_path=None
):
    ...
    for fn in swrc_files:
        ...
        plot_mesonet_swp_vs_vwc_with_violin(
            swrc_csv_path,
            vwc_parquet_path,
            save_path=out_png,
            show=show,
...
if __name__ == "__main__":
    ...
    plot_mesonet_dirs_swp_vs_vwc_with_violin(
        swrc_dir_, vwc_dir_, out_dir_, show=False, meta_csv_path=meta_csv_
    )
# ========================= EOF ====================================================================
