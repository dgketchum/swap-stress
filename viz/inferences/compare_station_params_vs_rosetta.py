...
def _find_rosetta_value(row, level, param):
    pat_plain = re.compile(rf"_L{int(level)}_VG_{param}$", re.IGNORECASE)
    pat_log = re.compile(rf"_L{int(level)}_VG_log10_{param}$", re.IGNORECASE)
    cols = row.index
...
def _load_polaris_by_station_level(polaris_parquet, id_col="station"):
    ...
    if id_col not in df.columns:
        return pd.DataFrame(
            columns=[id_col, "rosetta_level"]
        )  # likely error: polaris parquet missing expected id_col
    df = df.copy()
    if id_col == "station":
        df["station"] = (
            df["station"].astype(str).str.lower().str.replace("_", "-", regex=False)
        )
    if "rosetta_level" not in df.columns:
        if {"depth_min_cm", "depth_max_cm"}.issubset(df.columns):
            df["rosetta_level"] = [
                map_polaris_depth_range_to_rosetta_level(a, b)
                for a, b in zip(df["depth_min_cm"], df["depth_max_cm"])
            ]
        else:
            if "layer" in df.columns:
                ...
                df["rosetta_level"] = (
                    np.nan
                )  # likely error: missing depth info for POLARIS
...
def compare_station_params_vs_rosetta(
    training_parquet,
    out_dir,
    make_scatter=True,
    polaris_parquet=None,
    depth_cm=None,
    id_col="station",
):
    ...
    if pol.empty:
        raise ValueError(
            "POLARIS parquet loaded but empty; cannot compute combined metrics"
        )
    ...
    for i, p in enumerate(PARAMS):
        ...
        if make_scatter:
            ...
            if m is not None and f"pol_{p}" in m.columns:
                ...
                if not s_pol.empty:
                    vmin = min(
                        vmin, float(min(s_pol["true"].min(), s_pol["pred"].min()))
                    )
                    vmax = max(
                        vmax, float(max(s_pol["true"].max(), s_pol["pred"].max()))
                    )
            rng = vmax - vmin
            ...
            if s_pol is not None and not s_pol.empty:
                ...
                ann_lines.append(
                    f"POLARIS: n={len(s_pol)} R²={r2_pol:.2f} RMSE={rmse_pol:.3f}"
                )
            ax.text(
                0.02,
                0.98,
    ...
    if make_scatter:
        plt.tight_layout()
        plt.savefig(
            os.path.join(plot_dir, "station_vs_rosetta_all_params_metrics.png"), dpi=400
        )
        plt.close(fig)
    ...
    if metrics:
        ...
        comp.to_parquet(
            os.path.join(out_dir, "station_vs_rosetta_compact.parquet"), index=False
        )
    ...
    if m is not None:
        ...
        for p in PARAMS:
            ...
            bias = float(np.mean(s["pred"].values - s["true"].values))
            p_metrics.append(
                {"param": p, "n": len(s), "r2": r2, "rmse": rmse, "bias": bias}
            )
            pol_metrics_map[p] = {"n": len(s), "r2": r2, "rmse": rmse, "bias": bias}
        if p_metrics:
            pd.DataFrame(p_metrics).to_csv(
                os.path.join(out_dir, "station_vs_polaris_metrics.csv"), index=False
            )
        ...
        for p in PARAMS:
            ...
            row = {
                ...
                "n_ros": ros.get("n"),
                "r2_ros": ros.get("r2"),
                "rmse_ros": ros.get("rmse"),
                "bias_ros": ros.get("bias"),
                "n_pol": polm.get("n"),
                "r2_pol": polm.get("r2"),
                "rmse_pol": polm.get("rmse"),
        ...
        if rows_combined:
            ...
            out.to_csv(
                os.path.join(out_dir, "station_vs_references_metrics.csv"), index=False
            )
...
if __name__ == "__main__":
    ...
    depth_cm_ = 10.0  # set to numeric depth (cm) to filter by nearest station depth

    compare_station_params_vs_rosetta(
        training_pq_,
        out_dir_,
        make_scatter=True,
        polaris_parquet=polaris_pq_,
        depth_cm=depth_cm_,
        id_col="profile_id",
    )
# ========================= EOF ====================================================================
