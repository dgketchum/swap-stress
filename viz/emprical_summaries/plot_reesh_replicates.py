...
def plot_reesh_site_overlays(
    site_base, fits_dir, out_dir, show=True, colormap="plasma", max_panels=6
):
    files = [
        f
        for f in os.listdir(fits_dir)
        if f.startswith(site_base) and f.endswith(".json")
    ]
    if not files:
        return
    files = sorted(files)
    ...
    fig, axes = plt.subplots(
        nrows, ncols, figsize=(6 * ncols, 6.5 * nrows), sharey=True
    )
    ...
    for i, fn in enumerate(files):
        ...
        for c, depth_key in zip(colors, dk_sorted):
            ...
            if psi_obs.size and theta_obs.size:
                ax.plot(
                    ...
                    ms=4,
                    alpha=0.85,
                    label=f"{depth_key} cm",
                )
            params = entry["parameters"]
            tr = (
                params["theta_r"]["value"]
                if isinstance(params["theta_r"], dict)
                else params["theta_r"]
            )
            ts = (
                params["theta_s"]["value"]
                if isinstance(params["theta_s"], dict)
                else params["theta_s"]
            )
            al = (
                params["alpha"]["value"]
        ...
        if dk_sorted:
            ...
            tr_s = (
                sp["theta_r"]["value"]
                if isinstance(sp["theta_r"], dict)
                else sp["theta_r"]
            )
            ts_s = (
                sp["theta_s"]["value"]
                if isinstance(sp["theta_s"], dict)
                else sp["theta_s"]
            )
            al_s = (
                sp["alpha"]["value"] if isinstance(sp["alpha"], dict) else sp["alpha"]
            )
