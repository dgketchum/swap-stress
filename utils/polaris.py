...
def concat_polaris_stations(in_dirs, out_file):
        ...
        for col, var, d0, d1 in matches:
            ...
            if key in buckets:
                buckets[key] = buckets[key].merge(
                    tmp,
                    on=["station", "depth_min_cm", "depth_max_cm", "rosetta_level"],
                    how="outer",
                )
            else:
                buckets[key] = tmp
    ...
    cat = cat.groupby(
        ["station", "rosetta_level", "depth_min_cm", "depth_max_cm"], as_index=False
    ).mean()
...
def concat_polaris_gshp(in_dirs, out_file):
    ...
    cat = cat.groupby(
        ["profile_id", "rosetta_level", "depth_min_cm", "depth_max_cm"], as_index=False
    ).mean()
...
if __name__ == "__main__":
    ...
    if run_mt_mesonet_export:
        ...
        index_ = "station"
        output_prefix_ = (
            f"swapstress/polaris/mesonet_training_all_depths_{resolution_}m"
        )
