...
def plot_curves(
    ...
    ros_df = (
        pd.read_parquet(rosetta_parquet).groupby("profile_id").first().reset_index()
    )
    ...
    def polaris_level_for_depth(d_cm: float) -> Optional[int]:
        ...
        for lvl, (lo, hi) in POLARIS_DEPTH_RANGES.items():
            if lo <= dv < hi:
                return int(lvl)
        mids = {
            lvl: (rng[0] + rng[1]) / 2.0 for lvl, rng in POLARIS_DEPTH_RANGES.items()
        }
    ...
    for idx, r in d.groupby(["profile_id"]):
        ...
        pub_params = r.groupby("depth").first()[
            ["alpha_pub", "n_pub", "theta_r_pub", "theta_s_pub"]
        ]
        pub_by_depth: Dict[str, Dict[str, float]] = {}
        for depth_val, row in pub_params.iterrows():
            pub_by_depth[str(depth_val)] = {
                "theta_r": float(row.get("theta_r_pub"))
                if row.get("theta_r_pub") is not None
                else np.nan,
                "theta_s": float(row.get("theta_s_pub"))
                if row.get("theta_s_pub") is not None
                else np.nan,
                "alpha": float(row.get("alpha_pub"))
                if row.get("alpha_pub") is not None
                else np.nan,
                "n": float(row.get("n_pub"))
                if row.get("n_pub") is not None
                else np.nan,
            }
        ...
        for color, depth_cm in zip(colors, depths):
            ...
            if pub:
                tr = (
                    float(pub.get("theta_r"))
                    if pub.get("theta_r") is not None
                    else np.nan
                )
                ts = (
                    float(pub.get("theta_s"))
                    if pub.get("theta_s") is not None
                    else np.nan
                )
                a_pub = (
                    float(pub.get("alpha")) if pub.get("alpha") is not None else np.nan
                )
                ...
                if not (
                    ...
                    ax.plot(
                        ...
                        color=color,
                        lw=2,
                        label=f"{label_prefix} published",
                    )
                    ...
                    for col in cands:
                        if token in col and col in row.index:
                            ...
                            if isinstance(col, str) and (
                                "log10_" in col
                                or col.endswith("_log10_alpha")
                                or col.endswith("_log10_n")
                            ):
                                return float(10**val)
                ...
                if not (
                    ...
                    try:
                        rosetta_by_depth[float(depth_cm_)] = {
                            "theta_r": float(tr_ros),
                            "theta_s": float(ts_ros),
                            "alpha": float(a_ros),
                            "n": float(n_ros),
                            "level": level,
                        }
        ...
        for _pid, rp in raw_points_map.items():
            ...
            if s is None:
                continue
            suctions.extend(
                [float(v) for v in s if v is not None and np.isfinite(v) and v > 0]
            )
        if suctions:
            sf = np.sqrt(10.0)
        ...
        if shallow_vals:
            ...
            sh_color = colors[0]
            _pol_lvl = polaris_level_for_depth(
                sh_depth
            )  # used for consistent depth selection across sources
...
if __name__ == "__main__":
    ...
    plot_curves(
        gshp_csv_, rosetta_parquet_, plots_dir_, sample_uids=None, depth_cm=depth_cm_
    )
# ========================= EOF ====================================================================
