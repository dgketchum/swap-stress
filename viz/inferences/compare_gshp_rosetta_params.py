"""Compare GSHP parameters against Rosetta extractions.

Loads GSHP published parameters and Rosetta point extractions, aligns
columns flexibly, and produces distribution plots, optional scatter, and
summary metrics (R²/RMSE/bias). Also emits a merged Parquet table.
"""
...
def compare_gshp_to_rosetta(
    ...
    if merged is not None and not merged.empty:
        ...
        for p in PARAMS:
            ...
            rows.append(
                {"param": p, "n": len(dfp), "r2": r2, "rmse": rmse, "bias": bias}
            )
            ...
            if make_scatter:
                ...
                ax.set_ylabel(
                    f"Rosetta {'log10(' + p + ')' if p in LOG10_PARAMS else p}"
                )
                ax.set_title(f"GSHP vs Rosetta: {p} (R2={r2:.2f}, RMSE={rmse:.3f})")
                plt.tight_layout()
                out_fp = os.path.join(
                    out_dir, "plots", f"{p}_scatter_gshp_vs_rosetta.png"
                )
                plt.savefig(out_fp, dpi=300)
                plt.close(fig)
        ...
        if rows:
            ...
            metrics_df.to_csv(
                os.path.join(out_dir, "gshp_rosetta_metrics.csv"), index=False
            )
        ...
        merged[keep_cols].to_parquet(
            os.path.join(out_dir, "gshp_rosetta_merged.parquet"), index=False
        )
...
if __name__ == "__main__":
    ...
    output_csv_ = os.path.join(
        root_, "soils", "soil_potential_obs", "gshp", "extracted_rosetta_points.parquet"
    )
    ...
    compare_gshp_to_rosetta(
        soil_csv_path_, output_csv_, out_dir=gshp_directory_, make_scatter=True
    )
# ======================== EOF ====================================================================
