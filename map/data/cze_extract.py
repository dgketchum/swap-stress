...
def build_buffered_fc_from_points(
    shapefile: str, id_col: str = "profile_id", buffer_m: float = 250.0
    ...
    for _, row in gdf.iterrows():
        ...
        props = {
            id_col: row[id_col] if id_col in row and row[id_col] is not None else None
        }
    ...
    for year, month in dtimes:
        try:
            ...
            try:
                ...
                print(
                    f"ERA5 {year}-{str(month).zfill(2)} failed building date list: {e}"
                )
                continue
            ...
            for day_date in days_in_month_list:
                try:
                    ...
                    # SWE mm
                    swe = (
                        coll_day.select("snow_depth_water_equivalent")
                        .mean()
                        .multiply(1000)
                    ...
                    if RefETDaily is not None:
                        eto = RefETDaily.era5_land(coll_day).etr.rename(
                            f"eto_{day_str}"
                        )
                    ...
                    # Precip mm
                    precip = (
                        coll_day.select("total_precipitation_hourly")
                        .sum()
                        .multiply(1000)
                    ...
                    # Shortwave W/m^2
                    srad = (
                        coll_day.select("surface_solar_radiation_downwards_hourly")
                        .mean()
                    ...
                    if first_band_in_month:
                        ...
                        monthly_bands_image = monthly_bands_image.addBands(
                            ee.Image(daily_bands)
                        )
                except Exception as e:
...
def export_landsat_bands_over_buffers(
    ...
    if check_dir:
        if not os.path.isdir(check_dir):
            raise ValueError(
                f"File checking on but directory does not exist: {check_dir}"
            )
    ...
    for idx, row in gdf.iterrows():
        try:
            ...
            if geom.geom_type.lower() == "point":
                ...
                ee_geom = (
                    ee.Geometry.Point([lon, lat]).buffer(float(buffer_m))
                    if buffer_m and buffer_m > 0
                    else ee.Geometry.Point([lon, lat])
                )
            else:
                ...
                try:
                    ...
                    if geom.geom_type.lower() in ["polygon", "multipolygon"]:
                        ...
                        if geom.geom_type.lower() == "polygon":
                            ...
                            coords = [
                                [list(tup) for tup in list(largest.exterior.coords)]
                            ]
            ...
            for year in range(start_yr, end_yr + 1):
                ...
                if debug:
                    try:
                        sample = (
                            stacked.sample(fc_single, 30)
                            .first()
                            .toDictionary()
                            .getInfo()
                        )
                        print(
                            f"{desc}: sample keys (truncated):",
                            list(sample.keys())[:10],
                        )
                    except Exception as e:
                ...
                try:
                    data = stacked.reduceRegions(
                        collection=fc_single, reducer=ee.Reducer.mean(), scale=30
                    )
                except Exception as e:
    ...
    if check_dir:
        print(
            f"Landsat bands: Exported {exported}, skipped {skipped} files found in {check_dir}"
        )
